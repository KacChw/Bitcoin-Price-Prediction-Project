...
# 4. Rysowanie heatmapy korelacji (z poprawioną wielkością liter w 'coolwarm')
...
# Szukamy kombinacji od 1 do 4 zmiennych
for r in range(1, 5):
...
# 1. Przeprowadzamy PCA dla 2 głównych składowych
pca = PCA(n_components=2)
pca_results = pca.fit_transform(df_scaled)

# Tworzymy nową ramkę danych z wynikami współrzędnych dla każdego kraju
df_pca = pd.DataFrame(data=pca_results, columns=['PC1', 'PC2'])
df_pca.insert(0, 'Kraj', kraje)

# 2. Procent wyjaśnionej wariancji
...
# 3. Analiza ładunków czynnikowych (loadings) - co reprezentują osie PC1 i PC2
...
# 4. WYKRES: Wizualizacja krajów w przestrzeni PC1 i PC2
plt.figure(figsize=(13, 9))
plt.scatter(df_pca['PC1'], df_pca['PC2'], color='teal', edgecolors='black', s=120, alpha=0.8)

# Dodanie etykiet (nazw krajów) obok punktów
for i, txt in enumerate(df_pca['Kraj']):
    plt.annotate(txt, (df_pca['PC1'].iloc[i] + 0.08, df_pca['PC2'].iloc[i] + 0.08), fontsize=10, weight='bold')

# Linie osi układu współrzędnych (punkt zero)
plt.axhline(0, color='grey', linestyle='--', linewidth=1)
plt.axvline(0, color='grey', linestyle='--', linewidth=1)

plt.xlabel(f'Pierwsza składowa główna PC1 ({wyjasniona_wariancja[0]*100:.1f}%)', fontsize=12)
plt.ylabel(f'Druga składowa główna PC2 ({wyjasniona_wariancja[1]*100:.1f}%)', fontsize=12)
plt.title('Kraje UE w przestrzeni dwóch pierwszych składowych głównych (PCA)', fontsize=15, pad=15)
...
# 1. Klasyfikacja zmiennych na stymulanty i destymulanty
...
# Pracujemy na danych ustandaryzowanych (df_scaled) dla wybranych zmiennych
zmienne_ranking = stymulanty + destymulanty
df_ranking_input = df_scaled[zmienne_ranking]

# 2. Wyznaczenie wektora wzorca (z0)
wzorzec = {}
...
# 3. Obliczenie odległości euklidesowych od wzorca (d_i)
odleglosci = np.sqrt(((df_ranking_input - wzorzec_series) ** 2).sum(axis=1))

# 4. Obliczenie wartości miernika Hellwiga (m_i)
...
# 5. Stworzenie tabeli wynikowej i sortowanie
...
# 1. Unitaryzacja zerowana (Min-Max) na surowych danych numerycznych
df_min_max = zmienne_numeryczne[zmienne_ranking].copy()
for col in zmienne_ranking:
    min_val = df_min_max[col].min()
    max_val = df_min_max[col].max()
    # Zabezpieczenie przed dzieleniem przez 0, gdyby max == min
    if max_val != min_val:
        df_min_max[col] = (df_min_max[col] - min_val) / (max_val - min_val)
    else:
        df_min_max[col] = 0.0

# 2. Definiowanie wzorca i antywzorca w przestrzeni [0, 1]
# Dla stymulant: najlepsza wartość to 1, najgorsza to 0
# Dla destymulant: najlepsza wartość to 0, najgorsza to 1
wzorzec_topsis = {}
antywzorzec_topsis = {}
...
# 3. Obliczenie odległości euklidesowych od wzorca (d_plus) i antywzorca (d_minus)
d_plus = np.sqrt(((df_min_max - w_series) ** 2).sum(axis=1))
d_minus = np.sqrt(((df_min_max - a_series) ** 2).sum(axis=1))

# 4. Obliczenie współczynnika relatywnej bliskości (S_i)
wspolczynnik_topsis = d_minus / (d_plus + d_minus)

# 5. Stworzenie rankingu TOPSIS
...
# 1. Wybieramy te same zmienne ustandaryzowane, które służyły do rankingów
X_clustering = df_scaled[zmienne_ranking]

# 2. Obliczamy macierz powiązań metodą Warda (używając odległości euklidesowej)
linked = linkage(X_clustering, method='ward')

# 3. Rysowanie dendrogramu
...
# 1. Inicjalizacja algorytmu dla 3 skupień (na podstawie analizy dendrogramu)
LICZBA_SKUPIEN = 3
kmeans = KMeans(n_clusters=LICZBA_SKUPIEN, random_state=42, n_init=10)
clusters = kmeans.fit_predict(X_clustering)

# 2. Dodanie informacji o grupie do naszej głównej ramki danych
...
# Wyświetlamy kraje przypisane do poszczególnych grup
...
# 3. Profilowanie grup - obliczamy średnie wartości dla kluczowych zmiennych w każdej grupie
# Pokazuje to, czym charakteryzuje się dana grupa
...
# 1. Inicjalizacja i dopasowanie modelu dla 2 ukrytych czynników
fa = FactorAnalysis(n_components=2, random_state=42)
fa_results = fa.fit_transform(df_scaled)

# 2. Tworzenie macierzy ładunków czynnikowych (jak mocno zmienne budują dany czynnik)
...
# 3. Krótka pomoc w interpretacji nazw czynników
...
# 1. Obliczamy wartości własne (eigenvalues)
# W scikit-learn są one schowane pod nazwą 'explained_variance_'
wartosci_wlasne = pca.explained_variance_

# Ponieważ wcześniej ograniczyliśmy PCA do 2 komponentów, musimy na chwilę 
# przeliczyć pełne PCA dla wszystkich zmiennych, żeby zobaczyć pełne osypisko
full_pca = PCA()
